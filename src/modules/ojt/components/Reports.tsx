...
import {
  loadOjtReportSettings,
  saveOjtReportSettings,
  loadOjtReports,
  generateOjtReport,
  getOjtNextRunDate,
  downloadOjtReport,
  saveOjtReports,
  checkAndRunOjt,
  type OjtReportSettings,
  type OjtGeneratedReport,
...
export function OjtReports() {
  const [settings, setSettings] = useState<OjtReportSettings>(() => loadOjtReportSettings());
  const [saved, setSaved] = useState(false);
  const [reports, setReports] = useState<OjtGeneratedReport[]>(() => loadOjtReports());
  const [justGenerated, setJustGenerated] = useState(false);
  const [downloadError, setDownloadError] = useState("");

  // Run auto-check on mount
  useEffect(() => {
    const result = checkAndRunOjt();
    if (result) setReports(loadOjtReports());

    // Poll every 60 s while the page is open
    const id = setInterval(() => {
      const r = checkAndRunOjt();
      if (r) setReports(loadOjtReports());
    }, 60_000);
    return () => clearInterval(id);
  }, []);

  const nextRun = getOjtNextRunDate(settings);

  const handleSave = () => {
    saveOjtReportSettings(settings);
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  };

  const handleGenerateNow = () => {
    generateOjtReport(settings);
    setReports(loadOjtReports());
  ...
  const handleDeleteReport = (id: string) => {
    ...
    saveOjtReports(updated);
    setReports(updated);
  };

  const handleDownloadReport = async (report: OjtGeneratedReport) => {
    try {
      setDownloadError("");
      await downloadOjtReport(report);
    } catch (error) {
    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Auto-generate and manage OJT student reports on a schedule.
        </p>
      </div>
